...
@Component({
  selector: 'app-m2-s3-d',
  templateUrl: './m2-s3-d.component.html',
  styles: [
  ]
})
export class M2S3DComponent implements OnInit {
  form: any = {};
...
*/
  ...
  upload3(): void {
    ...
    if (this.selectedFiles) {
      ...
      if (file) {
        ...
        this.coursService.uploadCours2S3(this.currentFile,this.form).subscribe(
          (event: any) => {
            if (event.type === HttpEventType.UploadProgress) {
              this.progress = Math.round(100 * event.loaded / event.total);
            } else if (event instanceof HttpResponse) {
              this.message = event.body.message;
              this.fileInfos = this.coursService.getFilesCours2S3();
            }
          },
          (err: any) => {
            console.log(err);
  ...
  showFiles3(enable: boolean) {
    ...
    if (enable) {
      this.fileUploads = this.coursService.getFilesCours2S3();
    }
  }
