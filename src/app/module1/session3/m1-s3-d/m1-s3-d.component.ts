...
@Component({
  selector: 'app-m1-s3-d',
  templateUrl: './m1-s3-d.component.html',
  styles: [
  ]
})
export class M1S3DComponent implements OnInit {
...
*/
  ...
  upload3(): void {
    ...
    if (this.selectedFiles) {
      ...
      if (file) {
        ...
        this.coursService.uploadCours1S3(this.currentFile,this.form).subscribe(
          (event: any) => {
            if (event.type === HttpEventType.UploadProgress) {
              this.progress = Math.round(100 * event.loaded / event.total);
            } else if (event instanceof HttpResponse) {
              this.message = event.body.message;
              this.fileInfos = this.coursService.getFilesCours1S3();
            }
          },
          (err: any) => {
            console.log(err);
  ...
  showFiles3(enable: boolean) {
    ...
    if (enable) {
      this.fileUploads = this.coursService.getFilesCours1S3();
    }
  }
