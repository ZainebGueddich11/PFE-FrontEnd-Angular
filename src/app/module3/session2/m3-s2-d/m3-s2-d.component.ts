...
@Component({
  selector: 'app-m3-s2-d',
  templateUrl: './m3-s2-d.component.html',
  styles: [
  ]
})
export class M3S2DComponent implements OnInit {
 ...
 /* upload(): void {
  ...
  upload2(): void {
    ...
    if (this.selectedFiles) {
      ...
      if (file) {
        ...
        this.coursService.uploadCours3S2(this.currentFile,this.form).subscribe(
          (event: any) => {
            if (event.type === HttpEventType.UploadProgress) {
              this.progress = Math.round(100 * event.loaded / event.total);
            } else if (event instanceof HttpResponse) {
              this.message = event.body.message;
              this.fileInfos = this.coursService.getFilesCours3S2();
            }
          },
          (err: any) => {
            console.log(err);
  ...
  showFiles2(enable: boolean) {
    ...
    if (enable) {
      this.fileUploads = this.coursService.getFilesCours3S2();
    }
  }
