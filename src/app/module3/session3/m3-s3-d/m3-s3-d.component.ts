...
@Component({
  selector: 'app-m3-s3-d',
  templateUrl: './m3-s3-d.component.html',
  styles: [
  ]
})
export class M3S3DComponent implements OnInit {
  form: any = {};
...
*/
  ...
  upload3(): void {
    ...
    if (this.selectedFiles) {
      ...
      if (file) {
        ...
        this.coursService.uploadCours3S3(this.currentFile,this.form).subscribe(
          (event: any) => {
            if (event.type === HttpEventType.UploadProgress) {
              this.progress = Math.round(100 * event.loaded / event.total);
            } else if (event instanceof HttpResponse) {
              this.message = event.body.message;
              this.fileInfos = this.coursService.getFilesCours3S3();
            }
          },
          (err: any) => {
            console.log(err);
  ...
  showFiles3(enable: boolean) {
    ...
    if (enable) {
      this.fileUploads = this.coursService.getFilesCours3S3();
    }
  }
