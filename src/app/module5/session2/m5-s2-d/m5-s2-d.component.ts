...
@Component({
  selector: 'app-m5-s2-d',
  templateUrl: './m5-s2-d.component.html',
  styles: [
  ]
})
export class M5S2DComponent implements OnInit {
  form: any = {};
  ...
  upload2(): void {
    ...
    if (this.selectedFiles) {
      ...
      if (file) {
        ...
        this.coursService.uploadCours5S2(this.currentFile,this.form).subscribe(
          (event: any) => {
            if (event.type === HttpEventType.UploadProgress) {
              this.progress = Math.round(100 * event.loaded / event.total);
            } else if (event instanceof HttpResponse) {
              this.message = event.body.message;
              this.fileInfos = this.coursService.getFilesCours5S2();
            }
          },
          (err: any) => {
            console.log(err);
  ...
  showFiles2(enable: boolean) {
    ...
    if (enable) {
      this.fileUploads = this.coursService.getFilesCours5S2();
    }
  }
